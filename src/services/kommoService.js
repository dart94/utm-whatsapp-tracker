...
class KommoService {
 ...
 async createLead(leadData) {
  ...
  try {
    ...
    // PASO 5: Preparar custom fields
    const customFields = [];

    // SIEMPRE agregar TODOS los campos (aunque estén vacíos)
    if (fields.utmSource) {
      customFields.push({
        field_id: parseInt(fields.utmSource, 10),
        values: [{ value: utmSource || "" }],
      });
      console.log("Adding UTM Source:", utmSource || "(empty)");
    }
    ...
    if (fields.utmMedium) {
      ...
      console.log("Adding UTM Medium:", utmMedium || "(empty)");
    }
    ...
    if (fields.utmCampaign) {
      ...
      console.log("Adding UTM Campaign:", utmCampaign || "(empty)");
    }
    ...
    if (fields.utmContent) {
      ...
      console.log("Adding UTM Content:", utmContent || "(empty)");
    }
    ...
    if (fields.utmTerm) {
      ...
      console.log("Adding UTM Term:", utmTerm || "(empty)");
    }

    if (fields.fbclid && fbclid) {
      customFields.push({
        field_id: parseInt(fields.fbclid, 10),
        values: [{ value: fbclid }],
      });
      console.log("Adding FBCLID:", fbclid);
    }

    console.log("Total custom fields:", customFields.length);

    // PASO 6: Actualizar lead con custom fields
    if (customFields.length > 0) {
      console.log("Updating lead with custom fields...");
      
      // ✅ CORRECCIÓN AQUÍ - Usar paréntesis normales
      const updateResponse = await this._makeRequestWithRetry(() =>
